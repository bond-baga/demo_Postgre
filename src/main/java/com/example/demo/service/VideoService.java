...
//					mp.put("filename",Paths.get("file:///", demoapp.rootDir.toString(), "videofiles", String.format("%06d", form.getLoginid()), (String)tmp.get(i).get("filename")));					
					mp.put("filename",Paths.get("videofiles", String.format("%06d", form.getLoginid()), (String)tmp.get(i).get("filename")));					
	...
	public Boolean update(VideoForm form) throws Exception{
		for (int i = 0; i < form.getItems().size(); i++) {
			...
			if (form.getStates().get(i).isDelete()) {
				...
				// FileDelete
				Files.delete(Paths.get(form.getItems().get(i).get("filename").toString()));				
